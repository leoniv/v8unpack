...
int CV8File::SaveBlockData(std::basic_ofstream<char> &file_out, BYTE *pBlockData, UINT BlockDataSize, UINT PageSize)
{
    ...
    file_out.write(reinterpret_cast<char *>(pBlockData), BlockDataSize);
...
int CV8File::Parse(const std::string &filename_in, const std::string &dirname, int level)
{
    ...
    size_t sz_r;
    sz_r = file_in.readsome(reinterpret_cast<char*>(pFileData), FileDataSize);
    if (sz_r != FileDataSize) {
        std::cerr << "UnpackToFolder. Error in reading file!" << std::endl;
        return sz_r;
    }
...
int CV8File::LoadFileFromFolder(const std::string &dirname)
{
    ...
    for (; dit != d_end; ++dit) {
        ...
        if (boost::filesystem::is_directory(current_file)) {
            ...
            boost::filesystem::path p_filename(dirname);
            p_filename /= name;
